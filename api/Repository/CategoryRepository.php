...
class CategoryRepository extends EntityRepository {
    ...
    public function find($idcategory): ?Category{
        /*
            ...
            d'injection SQL.
        */
        $requete = $this->cnx->prepare("select * from CATEGORIES where id_categorie=:value"); // prepare la requête SQL
        $requete->bindParam(':value', $idcategory); // fait le lien entre le "tag" :value et la valeur de $id
        $requete->execute(); // execute la requête
        $answer = $requete->fetch(PDO::FETCH_OBJ);
        
        if ($answer==false) return null; // may be false if the sql request failed (wrong $id value for example)
        
        $p = new Category($answer->id_categorie);
    ...
    public function findAll(): array {
        ...
        foreach($answer as $obj){
            $p = new Category($obj->id_categorie);
            $p->setName($obj->nom);
            array_push($res, $p);
        }
    ...
    public function save($category){
        ...
        if ($answer){
            ...
            $category->setIdcategory($id); // set the product id to its real value.
            return true;
        }
