...
class ProductCollection {
    ...
    async load(uri){
        ...
        for(let item of objects){
            let p = new Product(item.id_produit, item.nom, item.image, item.prix, item.id_categorie, item.description, item.quantite, item.taille_vetement, item.couleur_vetement, item.bluray, item.langue, item.taille_vetement, item.couleur_vetement);
            this.#add(p);
        }
